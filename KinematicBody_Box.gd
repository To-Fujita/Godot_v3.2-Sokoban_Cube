...
func _process(delta):
	...
	if MoveVector == "L" and Result_XP == Player:
		...
		Direction = Vector3(0, 0, 0)
		var Temp_X = Box.global_transform.origin.x
		var Temp_Z = Box.global_transform.origin.z
		Box.global_transform.origin.x = floor(Temp_X / TileSize) * TileSize + TileSize / 2.0
		Box.global_transform.origin.z = floor(Temp_Z / TileSize) * TileSize + TileSize / 2.0
	...
	if Result_YM != null:
		if (Result_YM.name.substr(0, 18) == "@KinematicBody_Box" or Result_YM.name.substr(0, 18) == "@KinematicBody_Obs") and Direction == Vector3(0, 0, 0):
			Box.global_transform.origin.x = Result_YM.global_transform.origin.x
			Box.global_transform.origin.y = Result_YM.global_transform.origin.y + 2
			Box.global_transform.origin.z = Result_YM.global_transform.origin.z
	if Result_YPF == null:
		Stack_No = 1
		if MoveFlag <= TileSize and Direction != Vector3(0, 0, 0):
			Box.move_and_slide(Direction, Vector3())
			MoveFlag = MoveFlag + delta * Speed
		elif MoveFlag > TileSize:
			MoveFlag = 0
			Direction = Vector3(0, 0, 0)
			var Temp_X = Box.global_transform.origin.x
#			var Temp_Y = Box.global_transform.origin.y
			var Temp_Z = Box.global_transform.origin.z
			Box.global_transform.origin.x = floor(Temp_X / TileSize) * TileSize + TileSize / 2.0
#			Box.global_transform.origin.y = floor(Temp_Y / TileSize) * TileSize
			Box.global_transform.origin.z = floor(Temp_Z / TileSize) * TileSize + TileSize / 2.0
	if !YM_ray.is_colliding() and !On_ground:
		translation.y += GRAVITY * delta
	if Result_YM != null:
		Temp_Direction = Result_YM.global_transform.origin
		if Result_YM.name.substr(0, 19) == "@KinematicBody_Goal":
			get_node("MeshInstance").hide()
			get_node("MeshInstance2").show()
			GoalFlag = 1
		else:
			get_node("MeshInstance").show()
			get_node("MeshInstance2").hide()
			GoalFlag = 0
	if Box.global_transform.origin.y < -20:
		Box.global_transform.origin.y = -20
